...
namespace ghosen.TP20
{
  public class TP20_Session
  {
    public static IEnumerable<ISO_TP.Message> ProcessFrames(IEnumerable<ghosen.CAN.Message> messages)
    {
      ...
      {
        var parsedFrame = Framing.FrameParser.Parse(message.RawData, awaitingConsecutive);
        if (parsedFrame == null)
        {
        ...
        switch (parsedFrame.FrameType)
        {
          case Framing.FrameType.Single:
            ...
            {
              // this is probably the end of a multi-frame
              ...
              rawDataCollector.AddRange(parsedFrame.RawData);
              if (rawDataCollector.Count >= awaitingLength)
              {
                yield return new ISO_TP.Message()
                {
                  ArbId = new ISO_TP.ArbitrationId(message.ArbId),
          ...
          case Framing.FrameType.Consecutive:
            ...
            else
            {
              awaitingConsecutive = true;
              // grab first data bytes
              awaitingLength = parsedFrame.RawData[0];
              rawDataCollector = new List<byte>(awaitingLength);
              rawDataCollector.AddRange(parsedFrame.RawData.Skip(1));
            }
            break;
  ...
  public class Framing
  {
    ...
    public class SingleFrame : Frame
    {
        ...
        : base(message)
      {
        ...
        Length = (byte)(message[2] & 0x0F);

        // skip the first two (frame type and length)
        RawData = message.Skip(3).Take(Length).ToArray();
      }
